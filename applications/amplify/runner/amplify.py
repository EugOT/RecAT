...
Usage:
  pixi run amplify                          # smoke: M=3, N=1,5,9
  pixi run amplify --m 10 --n 1,3,5,7       # bigger smoke
  pixi run amplify --m 30 --n 1,3,5,7,9,11  # full publishable run
  pixi run amplify --calibration <path>     # use a specific report
  pixi run amplify --model <model>          # different model
...
def find_latest_calibration_report(traces_root: Path) -> Optional[Path]:
    ...
    candidates = sorted(
        traces_root.glob("calibrate-*/report.json"), key=lambda p: p.stat().st_mtime, reverse=True
    )
...
def run_isolated_arm(
    ...
    for prob_i in range(m):
        ...
        print(
            f"[isolated] problem {prob_i + 1}/{m}  expr={problem.expression[:80]}{'...' if len(problem.expression) > 80 else ''}",
            file=sys.stderr,
        )
        ...
        with ThreadPoolExecutor(max_workers=parallel) as ex:
            ...
            for attempt_i in range(max_n):
                ...
                def fixed_template(_rng, _p=problem):
                    return _p
            ...
            for fut in as_completed(futures):
                ...
                print(
                    f"  {mark} attempt {attempt_i + 1}/{max_n}  reported={result.reported}  truth={problem.truth}  ({result.elapsed_s:.1f}s)",
                    file=sys.stderr,
                )
...
def run_non_isolated_arm(
    ...
    with ThreadPoolExecutor(max_workers=parallel) as ex:
        ...
        for fut in as_completed(futs):
            ...
            print(
                f"[non-isolated] problem {prob_i + 1}/{m}  passes {n_passed}/{max_n}  "
    ...
    try:
        with tempfile.TemporaryDirectory(prefix=f"amplify-{session_label}-") as tmp:
            ...
            for turn_i in range(max_n):
                ...
                policy_violation = calibrate.trace_policy_violation(conv_path)
                if policy_violation or returncode != 0:
                    answers[turn_i] = None
                else:
...
def run_amplify(args: argparse.Namespace) -> int:
    ...
    if args.calibration:
        ...
        if cal_path is None:
            print(
                "ERROR: no calibrate-*/report.json found in traces/. "
                "Run `pixi run calibrate` first or pass --calibration.",
                file=sys.stderr,
            )
            return 2
    ...
    print("=== amplify Pass 2: amplification experiment ===", file=sys.stderr)
    ...
    if args.skip_non_isolated:
        ...
        print(
            f"  invocations:  isolated={iso_invs}, non-isolated={nis_invs}, total={iso_invs + nis_invs}",
            file=sys.stderr,
        )
    ...
    if args.skip_non_isolated:
        print(f"{'N':>3}  {'predicted':>10}  {'isolated p̂':>12}  {'iso CI':>16}", file=sys.stderr)
        print(f"{'-':>3}  {'---------':>10}  {'-----------':>12}  {'------':>16}", file=sys.stderr)
        for n in n_values:
            ic = isolated_curve[n]
            iso_ci = f"[{ic['ci_low']:.3f},{ic['ci_high']:.3f}]"
            print(
                f"{n:>3}  {ic['predicted']:>10.3f}  {ic['p_hat']:>12.3f}  {iso_ci:>16}",
                file=sys.stderr,
            )
    else:
        print(
            f"{'N':>3}  {'predicted':>10}  {'isolated p̂':>12}  {'iso CI':>16}  {'non-iso p̂':>12}  {'non-iso CI':>16}",
            file=sys.stderr,
        )
        print(
            f"{'-':>3}  {'---------':>10}  {'-----------':>12}  {'------':>16}  {'----------':>12}  {'----------':>16}",
        ...
        for n in n_values:
            ...
            print(
                f"{n:>3}  {ic['predicted']:>10.3f}  {ic['p_hat']:>12.3f}  {iso_ci:>16}  {nc['p_hat']:>12.3f}  {nis_ci:>16}",
                file=sys.stderr,
            )
    print("", file=sys.stderr)
    ...
    if args.skip_non_isolated:
        report["non_isolated_arm_skip_reason"] = "Skipped via --skip-non-isolated."
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        "--skip-non-isolated",
        action="store_true",
        help="Skip the non-isolated arm and run only the isolated arm.",
    )
